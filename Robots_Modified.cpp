...
int Robot::GetBagCount() const // return # of things in robot bag
{
	return this->bag;
}
int Robot::GetXCoordinate() const // return x position of robot
{
	return xPos;
}

int Robot::GetYCoordinate() const // return y position of robot
{
	return yPos;
}

void Robot::PickAllThings() // Pick all the cell to bag
{
	while(!CellEmpty())
	{
		PickThing();
...
{
	stalled = false; // be stalled false so it lives again
	theRobotWindow->Redraw(this);
}
void Robot::SetBagCount(int bagCount)
{
	this->bag = bagCount;
}
